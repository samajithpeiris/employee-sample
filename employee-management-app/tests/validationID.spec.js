...
test('Form validation: empty employee ID', async ({ page }) => {
  await page.goto('http://localhost:3000');

  // Fill out the form with an empty Employee ID but valid data in other fields
  await page.fill('input[name="employeeId"]', '');
  ...
  page.once('dialog', async (dialog) => {
    expect(dialog.message()).toContain("Please enter employee's ID.");
    await dialog.accept();
  });
